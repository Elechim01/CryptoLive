...
//     Creating Timeline Whitch will be updated for every 15 Minutes
...
#warning("controllare che con il passare del tempo l'API sia funzionante")
//        MARK: Fetch Request
        let session = URLSession(configuration: .default)
        let respone = try await session.data(from: URL(string: APIURL)!)
        let cryptoData = try JSONDecoder().decode([Crypto].self, from: respone.0)
...
//API URL
fileprivate let APIURL = "https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=bitcoin&order=market_cap_desc&per_page=1008page=1&sparkline=true&price_change_percentage=7d"
...
//    MARK: Finding Minium Price Value and applying it to Graph so that it can draw from value 0
    func min() -> Double {
